...
from sqlSample import sqlSample
...
class PGGRunner:
    def __init__(self, dbname='', user='', password='', host='', port='', isCostTraining=True, latencyRecord=True,
                 latencyRecordFile="RecordFile.json",
                 psycopg2=None):
        """
        :param dbname:
        :param user:
    ...
    def getLatency(self, sql, sqlwithplan):
        ...
        if thisQueryCost / sql.getDPCost() < 1000000000:
            try:
                ...
                self.cur.execute(
                    "set max_parallel_workers_per_gather = " + str(config.max_parallel_workers_per_gather) + ";")
...
pgrunner = PGGRunner(config.dbName, config.userName, config.password, config.ip, config.port,
                     isCostTraining=config.isCostTraining, latencyRecord=config.latencyRecord,
                     latencyRecordFile=latencyRecordFile)
...
def db_info():
    ...
    pgrunner.cur.execute(
        "SELECT conrelid::regclass, confrelid::regclass, conkey, confkey FROM pg_constraint WHERE contype = 'f';")
